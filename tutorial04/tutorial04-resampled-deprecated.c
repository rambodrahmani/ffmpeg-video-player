/**
*
*   File:   tutorial04-resampled-deprecated.c
...
*           tutorial04-resampled.c for an implementation using new APIs.
*
...
int main(int argc, char * argv[])
{
    ...
    videoState->decode_tid = SDL_CreateThread(decode_thread, videoState);
...
void printHelpMenu()
{
    ...
    printf("Usage: ./tutorial04-resampled-deprecated <filename> <max-frames-to-decode>\n\n");
    printf("e.g: ./tutorial04-resampled-deprecated /home/rambodrahmani/Videos/video.mp4 200\n");
}
...
void alloc_picture(void * userdata)
{
    ...
    if (videoPicture->bmp)
    {
        // we already have an SDL_Overlay allocated, free memory
        SDL_FreeYUVOverlay(videoPicture->bmp);
    }

    // lock global screen mutex
    SDL_LockMutex(screen_mutex);

    // allocate a new SDL_Overlay with the specified specs
    videoPicture->bmp = SDL_CreateYUVOverlay(
                                   videoState->video_ctx->width,
                                   videoState->video_ctx->height,
                                   SDL_YV12_OVERLAY,
                                   screen
...
int queue_picture(VideoState * videoState, AVFrame * pFrame)
{
    ...
    if (videoPicture->bmp)
    {
        // lock SDL_Overlay for direct pixel access
        SDL_LockYUVOverlay(videoPicture->bmp);

        /* Deprecated, please refer to tutorial04-resampled.c for the new API */
        pict.data[0] = videoPicture->bmp->pixels[0];
        pict.data[1] = videoPicture->bmp->pixels[2];
        pict.data[2] = videoPicture->bmp->pixels[1];

        pict.linesize[0] = videoPicture->bmp->pitches[0];
        pict.linesize[1] = videoPicture->bmp->pitches[2];
        pict.linesize[2] = videoPicture->bmp->pitches[1];
        ...
        // unlock SDL_Overlay
        SDL_UnlockYUVOverlay(videoPicture->bmp);
...
void video_display(VideoState * videoState)
{
    ...
    if (videoPicture->bmp)
    {
        ...
        // overlay was created.
        SDL_DisplayYUVOverlay(videoPicture->bmp, &rect);
